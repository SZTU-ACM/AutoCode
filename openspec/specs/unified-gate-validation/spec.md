...
Establish a single, centralized implementation of QualityGate / AuditGate decisions shared by `problem_pack_polygon` and `problem_audit`, so gate semantics stay consistent and a corrupt manifest fails instead of silently permitting.
