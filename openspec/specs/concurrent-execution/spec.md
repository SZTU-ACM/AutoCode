...
Define how AutoCode runs its heavy subprocess pipelines (test generation, test verification, stress testing) with bounded concurrency, preserving per-test correctness, timeout, and failure semantics identical to the serial implementation.
...
The system SHALL use a concurrency limit that defaults to a safe value (4) and MUST NOT exceed available memory; exceeding is capped gracefully.
