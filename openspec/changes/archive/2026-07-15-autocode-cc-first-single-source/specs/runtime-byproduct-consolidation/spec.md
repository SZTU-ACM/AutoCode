...
The `.autocode/` directory SHALL be listed in `.gitignore` so the runtime store is never committed.

#### Scenario: Clean version control
- **WHEN** a problem directory is inside a git repository
- **THEN** `.autocode/` is ignored and the runtime store does not appear in `git status`
